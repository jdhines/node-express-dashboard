describe('log-viewer.js', () => {
  it('should display the message from the WebSocket server @log-viewer-onmessage', () => {
    assert(
      fs.existsSync(
        path.join(process.cwd(), 'public/javascripts/log-viewer.js')
      ),
    ...
    const onmessage = logViewer.findPropertyAssignment(
      'connection',
      'onmessage'
    );
    ...
    assert(
      matchObj(onmessage, onmessageAssignmentMatch),
      'Are you assigning a function handler to `connection.onmessage` with a param of `event`?'
    );
    ...
    assert(
      matchObj(onmessage, onmessageArrowFunction) ||
        matchObj(onmessage, onmessageHandlerFunction),
      'Are you assigning a handler function to `connection.onmessage`?'
    );
    ...
    assert(
      matchObj(onmessage, onmessageMatch),
      'Are you setting the `innerHTML` property of `logWindow` to `event.data`?'
    );
  });
});
